...
export function workshop(s: Phaser.Scene) {
  s.add.image(270, H / 2, "workshop-depth").setDisplaySize(640, 960).setScrollFactor(0).setDepth(-30);
  s.add.rectangle(270, H / 2, 540, H, 0x08141e, 0.25).setScrollFactor(0).setDepth(-29);
  for (let y = 0; y < WORLD_H + 330; y += 300) {
    for (const x of [12, 528])
      s.add.image(x, y, "rail").setDisplaySize(26, 330).setAlpha(0.25).setDepth(-20);
  }
  for (let i = 0; i < 3; i++)
    s.add.image(103, FLOOR + 45 + i * 36, "platform").setDisplaySize(270 + i * 8, 65).setTint(0x8796a2).setDepth(3);
  s.add.text(105, FLOOR + 29, "INÍCIO", { fontFamily: "Arial", fontSize: "18px", letterSpacing: 4, color: "#c7d8df" }).setOrigin(0.5).setDepth(6);
  for (const [index, title] of ["PREPARAÇÃO", "ESTRUTURA", "ACABAMENTO"].entries())
